...
// Import motion from framer-motion (assuming it's available in the environment)
...
// Define item variants for individual cards
const item = {
  ...
  return (
    <section className="py-20 px-4 bg-background">
      <div className="max-w-7xl mx-auto">
        {/* Package Cards - Use motion.div as the container */}
        <motion.div 
          className="grid md:grid-cols-3 gap-8 mb-12"
          variants={container}
          initial="hidden"
          whileInView="show" // Use whileInView so the animation triggers when this section scrolls into view
          viewport={{ once: true, amount: 0.2 }} // Only animate once, when 20% of element is visible
        >
          {packages.map((pkg) => (
            // Use motion.div for individual items
            <motion.div
              key={pkg.id}
              variants={item} // Apply the item animation
            >
              <Card
                className={`card-monastery cursor-pointer transition-all duration-300 transform hover:scale-[1.02] hover:shadow-xl ${
        ...
        {/* Booking Interface - Give a simple fade-in effect to the form */}
        {selectedPackage && (
          <motion.div 
            className="mb-12"
